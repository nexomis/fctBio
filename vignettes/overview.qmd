...
head_imatrix <- data.frame(test_enrich$get_i_matrix()[1L:20L, 1L:30L])

head_imatrix$Row <- rownames(head_imatrix)

# Transform to long format
head_imatrix <- tidyr::pivot_longer(
  head_imatrix,
  cols = -Row,
  names_to = "Column",
  values_to = "Value"
...
# Plot using ggplot2
ggplot2::ggplot(
  head_imatrix, ggplot2::aes(x = Column, y = Row, fill = as.factor(Value))
) +
...
```{r}
#| label: visualize_pvalue_matrix
head_pmatrix <- data.frame(test_enrich$get_p_matrix()[1L:20L, ])

head_pmatrix$Row <- rownames(head_pmatrix)

# Transform to long format
head_pmatrix <- tidyr::pivot_longer(
  head_pmatrix,
  cols = -Row,
  names_to = "Column",
  values_to = "Value"
)
head_pmatrix$Value[is.na(head_pmatrix$Value)] <- 1.0
# Plot using ggplot2
